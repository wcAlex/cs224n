...
class NMT(nn.Module):
    ...
    def __init__(self, embed_size, hidden_size, vocab, dropout_rate=0.2):
        ...
        self.decoder = nn.LSTM(input_size=embed_size + hidden_size, hidden_size=hidden_size, num_layers=1, bias=True)
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        for Y_t in torch.split(Y, 1, dim=0):
            Y_t_s = torch.squeeze(Y_t, 0)
            Ybar_t = torch.cat((Y_t_s, o_prev), 1)
            dec_state, dec_out, _ = self.step(Ybar_t, dec_state, enc_hiddens, enc_hiddens_proj, enc_masks)
            combined_outputs.append(dec_out)
            o_prev = dec_out
