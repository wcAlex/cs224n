...
def negSamplingLossAndGradient(
    ...
    gradCenterVec = -(1-y_hat) * outsideVectors[outsideWordIdx]
    # gradient on u_o
    gradOutsideVecs[outsideWordIdx] = np.dot(y_hat - 1, centerWordVec)

    for k in range(K):
        wk = indices[k+1]
        yk_hat = sigmoid(-np.dot(outsideVectors[wk], centerWordVec))
        loss += -np.log(yk_hat)
        gradCenterVec += (1.0-yk_hat)*outsideVectors[wk]
        # need to use += not =, since wk might have duplicates.
        gradOutsideVecs[wk] += (1.0-yk_hat)*centerWordVec
...
def skipgram(currentCenterWord, windowSize, outsideWords, word2Ind,
    ...
    for w in outsideWords:
        loss_m, gradCenterVecs_m, gradOutsideVectors_m = word2vecLossAndGradient(centerWordVec, word2Ind[w], outsideVectors, dataset)
        loss += loss_m
        gradCenterVecs[centerWordInd] += gradCenterVecs_m
        gradOutsideVectors += gradOutsideVectors_m
